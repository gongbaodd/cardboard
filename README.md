...
####A/B压力测试结果
如下是100人同时发出1000次请求的压力测试结果。

####UI测试结果
如图？是YSlow的评分，评分为Ａ。

##结论
基于WebGL的虚拟现实眼镜将目前最流行的两个技术接合，从功能上实现了利用设备的方向感应器渲染场景和用户的选中、点击操作。同时，利用本产品的开放API实现的三个示例，全景图片浏览、电影选座和简单的增强现实效果，证明本产品有很好的扩展性。

本课题实现期间完成了以下工作：

1. 实现了平行双目立体图像对的生成算法，分别渲染左右眼图像。
2. 实现了设备坐标系到用户坐标系的转换算法，方便虚拟空间坐标转换。
3. 利用AJAX技术实现异步交互，无刷新进行查询请求和接收响应信息，提升了用户体验。
4. 试用Node.js做服务器，无阻塞的事件处理，提高服务器单CPU的利用率。
5. 提供相对完整的自动化测试文档，方便二次开发。
6. 本文的源代码开放于https://github.com/gongbaodd/cardboard 。

本课题的展望如下

1. 仅仅实现选中、点击操作还是远远不够的，可以适当利用websocket实现远程遥控。
2. 本文中的NoodleFace模块实现了简单的增强现实效果，可以继续开发，加入新的功能。

##参考文献

##致谢
短暂的四年如白驹过隙一般，回顾这四年，也许是我目前经历过的最跌宕起伏四年了。

首先要感谢的是我的父母，是你们的远见和多年的省吃俭用使我走出了“工人村”。然后是邵虹老师，您是给我第一次参加全国比赛机会的人，过去的两年里，我或许顽皮，或许任性，您都能容忍，很高兴能够受到您的教诲。

感谢404寝室的同学们，感谢你们能够容忍这么一个拥有奇葩生物钟的我。感谢徐永全学长、姜琳彦同学在我迷茫之际的帮助。感谢李圣龙等人在我外出实习期间对我的关照。感谢沈阳工业大学网络管理中心，如果当初没有加入这个社团，我如今也不能对web开发如此了解。

最后，感谢计算机系的全体老师，十分荣幸能够成为你们的学生。
